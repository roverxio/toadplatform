...
contract EntryPoint is IEntryPoint {
    ...
    mapping(address => uint) public gasUsedByAddress;
    ...
    function _executeOp(UserOperation memory op) private returns(uint) {
        ...
        uint gasUsed = abi.decode(returnData, (uint));
        gasUsedByAddress[op.sender] += gasUsed;
    ...
    function getGasUsed(address targetContract) external view returns(uint) {
        return gasUsedByAddress[targetContract];
    }
}